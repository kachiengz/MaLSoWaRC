...
Soil moisture (given suction) & thus Soil Water Retention Curve  for soil (e.g. Loamy Sand), subjected to 	monotonic drying, is predicted using 
...
print(predicted_SWC)
predicted_SWC.to_csv('./Output_files/SVR_rbf_quad_linear_predicted_SWC_Drying1.csv', encoding='utf-8', index=False)
...
X = soilData.iloc[:, 2:3].values #This is Q ($m^3$/s) create x as matrix instead of vector
n_samples=len(X)
X=X.reshape(-1,1)
...
plt.tight_layout()
plt.savefig('./Output_files/SVR_Drying1 _SWC_C=1000.png', bbox_inches="tight")
plt.savefig('./Output_files/SVR_Drying1 _SWC_C=1000.pdf', bbox_inches="tight")
plt.show()
